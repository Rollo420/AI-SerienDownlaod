...
def load_and_filter_proxies():
    ...
    try:
        ...
        if "proxies" in data:
            ...
            log(
                f"WARNUNG: 'proxies'-Schlüssel nicht in {filepath} gefunden.", "warning"
            )
    except FileNotFoundError:
        log(
            f"FEHLER: Proxy-Datei '{filepath}' nicht gefunden. Fahre ohne Proxys fort.",
            "error",
        )
    except json.JSONDecodeError as e:
        log(
            f"FEHLER: Ungültiges JSON-Format in '{filepath}': {e}. Fahre ohne Proxys fort.",
            "error",
        )
    except Exception as e:
        log(
            f"Ein unerwarteter Fehler beim Laden der Proxys aufgetreten: {e}. Fahre ohne Proxys fort.",
            "error",
        )

    if not proxies_list:
        log(
            "WARNUNG: Keine gültigen Proxys gefunden oder geladen. Der Browser wird ohne Proxy gestartet.",
            "warning",
        )
    return proxies_list
...
def merge_ts_files(ts_file_paths, output_filepath, ffmpeg_exec_path):
    ...
    ts_files_directory = (
        os.path.dirname(ts_file_paths[0])
        if ts_file_paths
        else os.path.dirname(output_filepath)
    )
...
def get_current_video_progress(driver):
    ...
    try:
        video_element_exists = driver.execute_script(
        ...
        for iframe in iframes:
            try:
                ...
                if "z-index" in style and int(z_index) > 100:  # Hoher z-index
                    ...
                    if rect["width"] > driver.execute_script(
                        "return window.innerWidth * 0.8;"
                    ) and rect["height"] > driver.execute_script(
                ...
                if (
                    ...
                    try:
                        ...
                        for inner_sel in inner_play_close_selectors:
                            try:
                                inner_btn = WebDriverWait(driver, 1).until(
                ...
                if selector == "JS_play":
                    ...
                    try:
                        ...
                        if duration > 0 and current_time > 0.1 and not paused:
                            ...
                            log(
                                f"Video pausiert nach 'video'-Klick bei {current_time:.2f}/{duration:.2f}. Versuche erneuten JS-play()."
                            )
                    except (
                        TimeoutException,
                        NoSuchElementException,
...
def find_m3u8_urls(self):
    ...
    """
    m3u8_urls = set()
    all_video_resources = self.extract_segment_urls_from_performance_logs()
...
def main():
    ...
    try:
        driver = initialize_driver(
            headless=not args.no_headless, proxyAddresse=args.proxyAddresse
        )
        ...
        if success and sorted_ts_urls:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for i, ts_url in enumerate(sorted_ts_urls):
                    segment_filename = f"{os.path.basename(urlparse(ts_url).path)}.ts" #f"segment_{i:05d}.ts"
                    futures.append(
                        executor.submit(
            ...
            sys.exit(0)  # Beende das Skript hier, wenn nur die Segmente ausgegeben werden sollen
            ...
            if not downloaded_ts_files:
                ...
                if ffmpeg_executable:
                    ...
                    if merge_ts_files(
                        downloaded_ts_files, final_output_video_path, ffmpeg_executable
                    ):
